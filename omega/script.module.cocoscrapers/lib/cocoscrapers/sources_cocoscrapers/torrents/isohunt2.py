...
class source:
    ...
    def __init__(self):
        ...
        self.search_link = (
            "/torrent/?ihq=%s&fiht=2&age=0&Torrent_sort=seeders&Torrent_page=0"
        )
    ...
    def sources(self, data, hostDict):
        ...
        try:
            ...
            run_and_wait(self.get_sources, rows)
            logged = False
            for quality in self.item_totals:
                if self.item_totals[quality] > 0 and not logged:
                    logged = True
                    log_utils.log(
                        "#STATS - ISOHUNT2 found {0:2.0f} {1}".format(
                            self.item_totals[quality], quality
                        )
                    )
            if not logged:
                log_utils.log("#STATS - ISOHUNT2 found nothing")
            endTime = time()
    ...
    def get_sources(self, row):
        ...
        for items in data:
            try:
                ...
                try:
                    url = (
                        unquote_plus(re.search(r'(magnet.*?)["\']', result).group(1))
                        .replace("&amp;", "&")
                        .split("&tr")[0]
                        .replace(" ", ".")
                    )
                ...
                url = unquote_plus(
                    url
                )  # many links dbl quoted so we must unquote again
