        ...
        try:
            ...
            for quality in self.item_totals:
                if self.item_totals[quality] > 0:
                    log_utils.log(
                        "#STATS - KICKASS2 found {0:2.0f} {1}".format(
                            self.item_totals[quality], quality
                        )
                    )
                    logged = True
            if not logged:
                log_utils.log("#STATS - KICKASS2 found nothing")
            endTime = time()
    ...
    def get_sources(self, url):
        ...
        try:
            ...
            rows = client.parseDOM(
                results, "tr", attrs={"id": "torrent_latest_torrents"}
            )
        except Exception:
            source_utils.scraper_error("KICKASS2")
            return
    ...
    def sources_packs(
        ...
        try:
            ...
            for quality in self.item_totals:
                if self.item_totals[quality] > 0:
                    log_utils.log(
                        "#STATS - KICKASS2(pack) found {0:2.0f} {1}".format(
                            self.item_totals[quality], quality
                        )
                    )
                    logged = True
            if not logged:
                log_utils.log("#STATS - KICKASS2(pack) found nothing")
            endTime = time()
            log_utils.log(
                "#STATS - KICKASS2(pack) took %.2f seconds" % (endTime - startTime)
            )
            return self.sources
        except Exception:
            source_utils.scraper_error("KICKASS2")
