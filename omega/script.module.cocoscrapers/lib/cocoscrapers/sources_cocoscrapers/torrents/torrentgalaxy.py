...
class source:
    ...
    def __init__(self):
        ...
        }  # cfscrape has some old User-Agents that fail
    ...
    def sources(self, data, hostDict):
        ...
        try:
            ...
            rows = client.parseDOM(
                result, "div", attrs={"class": "tgxtablerow txlight"}
            )
            if not rows:
                return sources
        ...
        for row in rows:
            try:
                ...
                if not source_utils.check_title(
                    title, aliases, name, hdlr, year, years
                ):
                    continue
                ...
                try:
                    seeders = int(
                        re.search(r"Seeders.*?<b>(.*?)<", columns[10], re.I).group(1)
                    )
        ...
        for quality in self.item_totals:
            if self.item_totals[quality] > 0:
                logged = True
                log_utils.log(
                    "#STATS - TORRENTGALAXY found {0:2.0f} {1}".format(
        ...
        try:
            ...
            for quality in self.item_totals:
                if self.item_totals[quality] > 0:
                    logged = True
                    log_utils.log(
                        "#STATS - TORRENTGALAXY(pack) found {0:2.0f} {1}".format(
                            self.item_totals[quality], quality
                        )
                    )
            if not logged:
            ...
            endTime = time()
            log_utils.log(
                "#STATS - TORRENTGALAXY(pack) took %.2f seconds" % (endTime - startTime)
            )
            return self.sources
        except Exception:
            source_utils.scraper_error("TORRENTGALAXY")
    ...
    def get_sources_packs(self, link):
        try:
            result = self.scraper.get(link, headers=self.headers, timeout=5).text
            if not result:
                return
            rows = client.parseDOM(
                result, "div", attrs={"class": "tgxtablerow txlight"}
