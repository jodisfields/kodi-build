...
class source:
    ...
    def sources(self, data, hostDict):
        ...
        try:
            ...
            results = client.request(
                api_url, post=query_data, headers=headers, timeout=5
            )
            if not results:
                return sources
        ...
        for file in files.get("content"):
            try:
                ...
                url = (
                    unquote_plus(file.get("magnet"))
                    .replace("&amp;", "&")
                    .replace(" ", ".")
                )
                url = re.sub(
                    r"(&tr=.+)&dn=", "&dn=", url
                )  # some links on bitlord &tr= before &dn=
                url = url.split("&tr=")[0].split("&xl=")[0]
                hash = re.search(r"btih:(.*?)&", url, re.I).group(1)

                if not episode_title:  # filter for eps in movie query
                ...
                try:
                    size = file.get("size")
                    size = (
                        str(size) + " GB" if len(str(size)) <= 2 else str(size) + " MB"
                    )  # bitlord size is all over the place between MB and GB
        ...
        for quality in self.item_totals:
            if self.item_totals[quality] > 0:
                log_utils.log(
                    "#STATS - BITLORD found {0:2.0f} {1}".format(
                        self.item_totals[quality], quality
                    )
                )
                logged = True
        if not logged:
            log_utils.log("#STATS - BITLORD found nothing")
        endTime = time()
    ...
    def sources_packs(
        ...
        try:
            ...
            for url in queries:
                links.append(
                    (
                        ("%s%s" % (self.base_link, self.search_link % url)).replace(
                            "+", "-"
                        ),
                        url.replace("+", "-"),
                    )
            ...
            for quality in self.item_totals:
                if self.item_totals[quality] > 0:
                    log_utils.log(
                        "#STATS - BITLORD(pack) found {0:2.0f} {1}".format(
                            self.item_totals[quality], quality
                        )
                    )
                    logged = True
            if not logged:
                log_utils.log("#STATS - BITLORD(pack) found nothing")
            endTime = time()
            log_utils.log(
                "#STATS - BITLORD(pack) took %.2f seconds" % (endTime - startTime)
            )
            return self.sources
        except Exception:
            source_utils.scraper_error("BITLORD")
    ...
    def get_sources_packs(self, link, url):
        try:
            ...
            api_url = "%s%s" % (self.base_link, self.api_search_link)
            rjson = client.request(
                api_url, post=query_data, headers=self.headers, timeout=5
            )
            if not rjson:
                return
            files = jsloads(rjson)
    ...
    def _get_token_and_cookies(self):
        ...
        try:
            ...
            headers.update(
                {
                    "Cookie": post[4].replace("SameSite=Lax, ", ""),
