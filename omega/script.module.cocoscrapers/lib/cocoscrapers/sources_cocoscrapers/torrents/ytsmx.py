...
class source:
    ...
    def __init__(self):
        ...
        self.search_link = (
            "/api/v2/list_movies.json?query_term=%s"  # accepts imdb_id as query_term
        )
    ...
    def sources(self, data, hostDict):
        ...
        try:
            ...
            if (
                files.get("status") == "error"
                or files.get("data").get("movie_count") == 0
            ):
                return sources
            title_long = (
                files.get("data").get("movies")[0].get("title_long").replace(" ", ".")
            )
        ...
        for quality in self.item_totals:
            if self.item_totals[quality] > 0:
                log_utils.log(
                    "#STATS - YTSMX found %s %s" % (self.item_totals[quality], quality)
                )
                logged = True
        if not logged:
            log_utils.log("#STATS - YTSMX nothing found")
        endTime = time()
