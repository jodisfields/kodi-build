...
class source:
    ...
    def sources(self, data, hostDict):
        ...
        try:
            ...
            query = "%s %s" % (
                ...
                hdlr,
            )  # eztv has issues with dashes in titles
        ...
        for row in rows:
            try:
                ...
                if len(hash) != 40:  # eztv has some base32 encoded hashes
                    ...
                    log_utils.log(
                        'url with base32 hash:  "%s" ' % url,
                        __name__,
                        log_utils.LOGDEBUG,
                    )
                    url = re.sub(
                        re.search(r"btih:(.*?)(?:&|$)", url).group(1), hash, url
                    )
                    log_utils.log(
                        'url converted to hex40 hash:  "%s" ' % url,
                        __name__,
                        log_utils.LOGDEBUG,
                    )

                name = "".join(link[1].partition("[eztv]")[:2]).replace(
                    " Torrent: Magnet Link", ""
                )
        ...
        for quality in self.item_totals:
            if self.item_totals[quality] > 0:
                log_utils.log(
                    "#STATS - EZTV found {0:2.0f} {1}".format(
                        self.item_totals[quality], quality
                    )
                )
                logged = True
        if not logged:
            log_utils.log("#STATS - EZTV found nothing")
        return sources
    ...
    def sources_packs(
        ...
        sources = []
        if search_series:
            return sources  # eztz does not have showPacks
        if not data:
            return sources
        sources_append = sources.append
        try:
        ...
        for row in rows:
            try:
                ...
                name_info = source_utils.info_from_name(
                    name, title, year, season=season_x, pack=package
                )
                if source_utils.remove_lang(name_info, check_foreign_audio):
                    continue
                if undesirables and source_utils.remove_undesirables(
        ...
        for quality in self.item_totals:
            if self.item_totals[quality] > 0:
                log_utils.log(
                    "#STATS - EZTV(pack) found {0:2.0f} {1}".format(
                        self.item_totals[quality], quality
                    )
                )
                logged = True
        if not logged:
            log_utils.log("#STATS - EZTV(pack) found nothing")
        return sources
