...
class source:
    ...
    def sources(self, data, hostDict):
        ...
        try:
            ...
            if "tvshowtitle" in data:
                homeWindow.setProperty(
                    "cocoscrapers.torrentio.performing_single_scrape", "true"
                )
                title = (
                    data["tvshowtitle"]
                    .replace("&", "and")
        ...
        for file in files:
            try:
                ...
                homeWindow.clearProperty(
                    "cocoscrapers.torrentio.performing_single_scrape"
                )
                source_utils.scraper_error("TORRENTIO")
        logged = False
        for quality in self.item_totals:
            if self.item_totals[quality] > 0:
                log_utils.log(
                    "#STATS - TORRENTIO found {0:2.0f} {1}".format(
                        self.item_totals[quality], quality
                    )
                )
                logged = True
        if not logged:
            log_utils.log("#STATS - TORRENTIO found nothing")
        endTime = time()
    ...
    def sources_packs(
        ...
        for quality in self.item_totals:
            if self.item_totals[quality] > 0:
                log_utils.log(
                    "#STATS - TORRENTIO(pack) found {0:2.0f} {1}".format(
                        self.item_totals[quality], quality
                    )
                )
                logged = True
        if not logged:
            log_utils.log("#STATS - TORRENTIO(pack) found nothing")
        endTime = time()
        log_utils.log(
            "#STATS - TORRENTIO(pack) took %.2f seconds" % (endTime - startTime)
        )
        return sources
