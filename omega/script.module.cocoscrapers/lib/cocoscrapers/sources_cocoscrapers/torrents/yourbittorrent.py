...
class source:
    ...
    def sources(self, data, hostDict):
        ...
        try:
            ...
            if not results:
                return self.sources
            links = re.findall(
                r'<a\s*href\s*=\s*["\'](/torrent/.+?)["\']', results, re.DOTALL | re.I
            )
            ...
            for quality in self.item_totals:
                if self.item_totals[quality] > 0:
                    logged = True
                    log_utils.log(
                        "#STATS - YOURBITTORRENT found {0:2.0f} {1}".format(
                            self.item_totals[quality], quality
                        )
                    )
            if not logged:
                log_utils.log("#STATS - YOURBITTORRENT no results found")
            endTime = time()
            log_utils.log(
                "#STATS - YOURBITTORRENT took %.2f seconds" % (endTime - startTime)
            )
            return self.sources
        except Exception:
    ...
    def get_sources(self, link):
        try:
            ...
            name = (
                re.search(
                    r'<h3\s*class\s*=\s*["\']card-title["\']>(.+?)<', result, re.I
                )
                .group(1)
            ...
            try:
                seeders = int(
                    re.search(
                        r">Seeders:.*?>\s*([0-9]+|[0-9]+,[0-9]+)\s*</", result, re.I
                    )
                    .group(1)
                    .replace(",", "")
                )
        ...
        try:
            ...
            for quality in self.item_totals:
                if self.item_totals[quality] > 0:
                    logged = True
                    log_utils.log(
                        "#STATS - YOURBITTORRENT(pack) found {0:2.0f} {1}".format(
                            self.item_totals[quality], quality
                        )
                    )
            if not logged:
                log_utils.log("#STATS - YOURBITTORRENT(pack) no results found")
            endTime = time()
            log_utils.log(
                "#STATS - YOURBITTORRENT(pack) took %.2f seconds"
                % (endTime - startTime)
            )
            return self.sources
        except Exception:
