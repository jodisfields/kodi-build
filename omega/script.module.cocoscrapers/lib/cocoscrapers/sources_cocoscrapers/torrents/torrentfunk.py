...
class source:
    ...
    def __init__(self):
        ...
        self.tvsearch_link = (
            "/television/torrents/%s.html?v=on&smi=0&sma=0&i=75&sort=size&o=desc"
        )
        self.msearch_link = (
            "/movie/torrents/%s.html?v=on&smi=0&sma=0&i=75&sort=size&o=desc"
        )
    ...
    def sources(self, data, hostDict):
        ...
        try:
            ...
            run_and_wait(self.get_sources, links)
            logged = False
            for quality in self.item_totals:
                if self.item_totals[quality] > 0:
                    log_utils.log(
                        "#STATS -TORRENTFUNK found {0:2.0f} {1}".format(
                            self.item_totals[quality], quality
                        )
                    )
                    logged = True
            if not logged:
                log_utils.log("#STATS - TORRENTFUNK found nothing")
            endTime = time()
    ...
    def get_sources(self, link):
        try:
            ...
            try:
                seeders = int(
                    re.search(r"Swarm.+?>([0-9]+|[0-9]+,[0-9]+)</", link, re.I)
                    .group(1)
                    .replace(",", "")
                )
            ...
            try:
                size = re.search(
                    r"Size:.*?>((?:\d+\,\d+\.\d+|\d+\.\d+|\d+\,\d+|\d+)\s*(?:GB|GiB|Gb|MB|MiB|Mb))",
                    link,
                ).group(1)
    ...
    def sources_packs(
        ...
        try:
            ...
            for quality in self.item_totals:
                if self.item_totals[quality] > 0:
                    log_utils.log(
                        "#STATS - TORRENTFUNK(pack) found {0:2.0f} {1}".format(
                            self.item_totals[quality], quality
                        )
                    )
                    logged = True
            if not logged:
                log_utils.log("#STATS - TORRENTFUNK(pack) found nothing")
            endTime = time()
            log_utils.log(
                "#STATS - TORRENTFUNK(pack) took %.2f seconds" % (endTime - startTime)
            )
            return self.sources
        except Exception:
            source_utils.scraper_error("TORRENTFUNK")
            ...
            try:
                ...
                if not self.search_series:
                    if not self.bypass_filter:
                        valid, self.episode_start, self.episode_end = (
