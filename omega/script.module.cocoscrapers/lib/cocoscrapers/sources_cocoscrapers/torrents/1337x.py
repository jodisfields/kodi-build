...
from cocoscrapers.modules.Thread_pool import run_pipelined
...
class source:
    ...
    def __init__(self):
        ...
        self.domains = [
            ...
            "1337x.se",
            "1337x.is",
        ]  # all are behind cloudflare except .to
    ...
    def sources(self, data, hostDict):
        ...
        try:
            ...
            query = "%s %s" % (source_utils.sanitize_query(self.title), self.hdlr)
            urls = []
            ...
            run_pipelined(self.get_items, self.get_sources, urls)
            logged = False
            for quality in self.item_totals:
                if self.item_totals[quality] > 0:
                    log_utils.log(
                        "#STATS - 1337x.to found {0:2.0f} {1}".format(
                            self.item_totals[quality], quality
                        )
                    )
                    logged = True
            if not logged:
                log_utils.log("#STATS - 1337x.to found nothing")
            endTime = time()
    ...
    def get_items(self, url):
        ...
        for row in rows:
            try:
                ...
                link = "%s%s" % (
                    self.base_link,
                    client.parseDOM(columns[0], "a", ret="href")[1],
                )
                ...
                if not self.episode_title:  # filter for eps returned in movie query
                ...
                try:
                    ...
                    isize = "0"
                    dsize = 0
                items.append((name, name_info, link, isize, dsize, seeders))
            except Exception:
                source_utils.scraper_error("1337X")
        return items
