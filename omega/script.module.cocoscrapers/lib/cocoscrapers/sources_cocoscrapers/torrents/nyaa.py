...
class source:
    ...
    def sources(self, data, hostDict):
        ...
        try:
            ...
            hdlr2 = (
                "S%d - %d" % (int(data["season"]), int(data["episode"]))
                if "tvshowtitle" in data
                else year
            )

            query = "%s %s" % (source_utils.sanitize_query(title), hdlr)
            query2 = "%s %s" % (source_utils.sanitize_query(title), hdlr2)
            urls = []
            ...
            try:
                ...
                for row in rows:
                    links = zip(
                        ...
                        [
                            re.findall(
                                r'<td class\s*=\s*["\']text-center["\']>([0-9]+)</td>',
                                row,
                                re.DOTALL,
                            )
                        ],
        ...
        for quality in self.item_totals:
            if self.item_totals[quality] > 0:
                log_utils.log(
                    "#STATS - NYAA found {0:2.0f} {1}".format(
                        self.item_totals[quality], quality
                    )
                )
                logged = True
        if not logged:
            log_utils.log("#STATS - NYAA found nothing")
        endTime = time()
