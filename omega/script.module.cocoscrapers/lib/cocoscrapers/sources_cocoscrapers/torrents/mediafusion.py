...
class source:
    ...
    def _get_files(self, url):
        if control.setting("mediafusion_user_data") == "":
            ...
            headers = {"encoded_user_data": control.setting("mediafusion_user_data")}
        if self.get_pack_files:
            return []
    ...
    def sources(self, data, hostDict):
        ...
        sources = []
        if not data:
            homeWindow.clearProperty(
                "cocoscrapers.mediafusion.performing_single_scrape"
            )
            return sources
        ...
        try:
            ...
            if "tvshowtitle" in data:
                homeWindow.setProperty(
                    "cocoscrapers.mediafusion.performing_single_scrape", "true"
                )
                season = data["season"]
                ...
                url = "%s%s" % (
                    self.base_link,
                    self.tvSearch_link % (imdb, season, episode),
                )
        ...
        for quality in self.item_totals:
            if self.item_totals[quality] > 0:
                log_utils.log(
                    "#STATS - MEDIAFUSION found {0:2.0f} {1}".format(
        ...
        endTime = time()
        if not logged:
            log_utils.log("#STATS - MEDIASEARCH found nothing")
    ...
    def sources_packs(
        ...
        while count < 10000 and not finished_single_scrape:
            finished_single_scrape = (
                homeWindow.getProperty(
                    "cocoscrapers.mediafusion.performing_single_scrape"
                )
                != "true"
            )
        ...
        try:
            ...
            url = "%s%s" % (
                self.base_link,
                self.tvSearch_link % (imdb, season, data["episode"]),
            )
            files = cache.get(self._get_files, 10, url)
        ...
        for file in files:
            ...
            try:
                ...
                name_info = source_utils.info_from_name(
                    name, title, year, season=season, pack=package
                )
                if source_utils.remove_lang(name_info, check_foreign_audio):
                    continue
                if undesirables and source_utils.remove_undesirables(
        ...
        for quality in self.item_totals:
            if self.item_totals[quality] > 0:
                log_utils.log(
                    "#STATS - MEDIAFUSION(pack) found {0:2.0f} {1}".format(
                        self.item_totals[quality], quality
                    )
                )
                logged = True
        endTime = time()
        if not logged:
            log_utils.log("#STATS - MEDIAFUSION(pack) found nothing")
        log_utils.log(
            "#STATS - MEDIAFUSION(pack) took %.2f seconds" % (endTime - startTime)
        )
        return sources
