...
class source:
    ...
    def __init__(self):
        self.language = ["en"]
        self.base_link = (
            "https://www.torrentquest.com"  # torrentquest is mirror of magnetdl
        )
    ...
    def sources(self, data, hostDict):
        ...
        try:
            ...
            if "tvshowtitle" in data:
                title = (
                    data["tvshowtitle"]
                    .replace("&", "and")
                    .replace("Special Victims Unit", "SVU")
                    .replace("/", " ")
                    .replace("$", "s")
                )
                if (
                    title == "The End of the Fucking World"
                    or title == "The End of the F***ing World"
                ):
                    title = "The End of the Fxxxing World"
                ...
                if title == "The Fuck-It List" or title == "The F**k-It List":
                    title = "The Fxxk-It List"
                episode_title = None
                hdlr = year
            query = "%s %s" % (source_utils.sanitize_query(title), hdlr)
            url = "%s%s" % (
                self.base_link,
                self.search_link.format(query[0].lower(), cleantitle.geturl(query)),
            )
        ...
        try:
            ...
            if not results2 or "<tbody" not in results2:
                raise Exception()
        ...
        for row in rows:
            try:
                ...
                try:
                    url = (
                        re.search(r"(magnet:.+?)&tr=", url, re.I)
                        .group(1)
                        .replace(" ", ".")
                    )
                except Exception:
                ...
                if source_utils.remove_lang(name_info, check_foreign_audio):
                    continue
                if undesirables and source_utils.remove_undesirables(
                    name_info, undesirables
                ):
                    continue
        ...
        for quality in self.item_totals:
            if self.item_totals[quality] > 0:
                log_utils.log(
                    "#STATS - TORRENTQUEST found {0:2.0f} {1}".format(
                        self.item_totals[quality], quality
                    )
                )
                logged = True
        if not logged:
            log_utils.log("#STATS - TORRENTQUEST found nothing")
        endTime = time()
    ...
    def sources_packs(
        ...
        try:
            ...
            if (
                self.title == "The End of the Fucking World"
                or self.title == "The End of the F***ing World"
            ):
            ...
            query = source_utils.sanitize_query(title)
            if search_series:
                queries = [
                    self.search_link.format(
                        query[0].lower(), cleantitle.geturl(query + " Season")
                    ),
                    self.search_link.format(
                        query[0].lower(), cleantitle.geturl(query + " Complete")
                    ),
                ]
            else:
                queries = [
            ...
            for quality in self.item_totals:
                if self.item_totals[quality] > 0:
                    log_utils.log(
                        "#STATS - TORRENTQUEST(pack) found {0:2.0f} {1}".format(
                            self.item_totals[quality], quality
                        )
                    )
                    logged = True
            if not logged:
                log_utils.log("#STATS - TORRENTQUEST(pack) found nothing")
            endTime = time()
            log_utils.log(
                "#STATS - TORRENTQUEST(pack) took %.2f seconds" % (endTime - startTime)
            )
            return self.sources
        except Exception:
            source_utils.scraper_error("TORRENTQUEST")
            ...
            try:
                ...
                if not self.search_series:
                    if not self.bypass_filter:
                        valid, episode_start, episode_end = (
                            source_utils.filter_season_pack(
                ...
                name_info = source_utils.info_from_name(
                    name, self.title, self.year, season=self.season_x, pack=package
                )
                if source_utils.remove_lang(name_info, self.check_foreign_audio):
                    continue
                if self.undesirables and source_utils.remove_undesirables(
                    name_info, self.undesirables
                ):
                    continue
