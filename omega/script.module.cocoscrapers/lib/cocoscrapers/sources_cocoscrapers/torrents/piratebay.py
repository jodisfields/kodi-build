    ...
    def sources(self, data, hostDict):
        ...
        for file in files:
            try:
                ...
                try:
                    dsize, isize = source_utils.convert_size(
                        float(file["size"]), to="GB"
                    )
                    info.insert(0, isize)
                except Exception:
                    dsize = 0
        ...
        for quality in self.item_totals:
            if self.item_totals[quality] > 0:
                log_utils.log(
                    "#STATS - PIRATEBAY found {0:2.0f} {1}".format(
                        self.item_totals[quality], quality
                    )
                )
                logged = True
        if not logged:
            log_utils.log("#STATS - PIRATEBAY found nothing")
        endTime = time()
    ...
    def sources_packs(
        ...
        try:
            ...
            for quality in self.item_totals:
                if self.item_totals[quality] > 0:
                    log_utils.log(
                        "#STATS - PIRATEBAY(pack) found {0:2.0f} {1}".format(
                            self.item_totals[quality], quality
                        )
                    )
                    logged = True
            if not logged:
                log_utils.log("#STATS - PIRATEBAY(pack) found nothing")
            endTime = time()
            log_utils.log(
                "#STATS - PIRATEBAY(pack) took %.2f seconds" % (endTime - startTime)
            )
            return self.sources
        except Exception:
            source_utils.scraper_error("PIRATEBAY")
