        ...
        try:
            ...
            for quality in self.item_totals:
                if self.item_totals[quality] > 0:
                    log_utils.log(
                        "#STATS - TORRENTDOWNLOAD found {0:2.0f} {1}".format(
                            self.item_totals[quality], quality
                        )
                    )
                    logged = True
            if not logged:
                log_utils.log("#STATS - TORRENTDOWNLOAD found nothing")
            endTime = time()
            log_utils.log(
                "#STATS - TORRENTDOWNLOAD took %.2f seconds" % (endTime - startTime)
            )
            return self.sources
        except Exception:
    ...
    def get_sources(self, url):
        ...
        for row in rows:
            try:
                ...
                link = (
                    re.search(r'href\s*=\s*["\']/(.+?)["\']>', columns[0], re.I)
                    .group(1)
                    .split("/")
                )
                hash = link[0]
                name = source_utils.clean_name(
                    unquote_plus(link[1]).replace("&amp;", "&")
                )
    ...
    def sources_packs(
        ...
        try:
            ...
            for quality in self.item_totals:
                if self.item_totals[quality] > 0:
                    log_utils.log(
                        "#STATS - TORRENTDOWNLOAD(pack) found {0:2.0f} {1}".format(
                            self.item_totals[quality], quality
                        )
                    )
                    logged = True
            if not logged:
                log_utils.log("#STATS - TORRENTDOWNLOAD(pack) found nothing")
            endTime = time()
            log_utils.log(
                "#STATS - TORRENTDOWNLOAD(pack) took %.2f seconds"
                % (endTime - startTime)
            )
            return self.sources
        except Exception:
