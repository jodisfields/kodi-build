    ...
    def sources(self, data, hostDict):
        ...
        try:
            ...
            params = {
                ...
                "debridApiKey": debrid_token,
                "debridStreamProxyPassword": "",
            }
            params = base64.b64encode(
                jsdumps(params, separators=(",", ":")).encode("utf-8")
            ).decode("utf-8")
            if "tvshowtitle" in data:
                homeWindow.setProperty(
                    "cocoscrapers.comet.performing_single_scrape", "true"
                )
                season = data["season"]
                ...
                url = "%s%s" % (
                    self.base_link,
                    self.tvSearch_link % (params, imdb, season, episode),
                )
        ...
        for quality in self.item_totals:
            if self.item_totals[quality] > 0:
                log_utils.log(
                    "#STATS - COMET found {0:2.0f} {1}".format(
                        self.item_totals[quality], quality
                    )
                )
                logged = True
        if not logged:
            log_utils.log("#STATS - COMET found nothing")
        endTime = time()
    ...
    def sources_packs(
        ...
        while count < 10000 and not finished_single_scrape:
            finished_single_scrape = (
                homeWindow.getProperty("cocoscrapers.comet.performing_single_scrape")
                != "true"
            )
            sleep(100)
            count += 100
        ...
        try:
            ...
            url = "%s%s" % (
                self.base_link,
                self.tvSearch_link % (params, imdb, season, data["episode"]),
            )
            files = cache.get(self._get_files, 10, url)
        ...
        for quality in self.item_totals:
            if self.item_totals[quality] > 0:
                log_utils.log(
                    "#STATS - COMET(pack) found {0:2.0f} {1}".format(
                        self.item_totals[quality], quality
                    )
                )
                logged = True
        if not logged:
            log_utils.log("#STATS - COMET(pack) found nothing")
        endTime = time()
