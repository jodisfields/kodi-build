...
from cocoscrapers.modules.control import homeWindow, setting as getSetting

# Pre-compiled regex patterns for performance
_EPISODE_PATTERNS = [
    re.compile(r"[.-]s\d{2}e\d{2}([.-]?)"),
    re.compile(r"[.-]s\d{2}([.-]?)"),
    re.compile(r"[.-]season[.-]?\d{1,2}[.-]?"),
]
_QUERY_SANITIZE_RE = re.compile(r"[^A-Za-z0-9\s\.-]+")
_HASH_RE = re.compile(r"btih:(.*?)&", re.I)
_MAGNET_RE = re.compile(r'href\s*=\s*["\'](magnet:.+?)["\']', re.I)


def is_episode_in_name(name_lower):
    """Check if name contains episode patterns. Use pre-compiled regex."""
    return any(p.search(name_lower) for p in _EPISODE_PATTERNS)


def sanitize_query(title):
    """Remove special characters from title for search query."""
    return _QUERY_SANITIZE_RE.sub("", title)
...
def get_undesirables():
    if (
        not getSetting("filter.undesirables") == "true"
        or home_getProperty("fs_filterless_search") == "true"
    ):
        return []
    try:
...
def check_foreign_audio():
    return (
        False
        if home_getProperty("fs_filterless_search") == "true"
        else getSetting("filter.foreign.single.audio") == "true"
    )
...
def check_title(
    title, aliases, release_title, hdlr, year, years=None
):  # non pack file title check, single eps and movies
    ...
    try:
        ...
        release_title = re.sub(
            r"([(])(?=((19|20)[0-9]{2})).*?([)])", "\\2", release_title
        )  # remove parenthesis only if surrounding a 4 digit date
        t = (
            re.split(r"%s" % hdlr, release_title, 1, re.I)[0]
            .replace(year, "")
            .replace("&", "and")
        )
        if years:
            for i in years:
                t = t.split(i)[0]
        t = re.split(
            r"2160p|216op|4k|1080p|1o8op|108op|1o80p|720p|72op|480p|48op", t, 1, re.I
        )[0]
    ...
    try:
        ...
        if check_foreign_audio:
            if any(value in release_info for value in LANG) and not any(
                value in release_info for value in ENG_CHECK
            ):
                return True
        ...
        if release_info.endswith(".srt.") and not any(
            value in release_info for value in SRT_CHECK
        ):
            return True
        return False
    except Exception:
...
def filter_season_pack(show_title, aliases, year, season, release_title):
    ...
    try:
        ...
        season_fill_checke = (
            ".s%se" % season_fill
        )  # added 3/2/22 to pick up episode range packs ex "Reacher.s01e01-08"
        ...
        if any(i in rt for i in string_list):
            for item in (
                ...
                r"\Ws\d{2}\W%s"
                % season_fill_check.lstrip("."),  # may need more reverse ranges
...
def filter_show_pack(
    show_title, aliases, imdb, year, season, release_title, total_seasons
):
    aliases = aliases_to_array(aliases)
    title_list = []
    title_list_append = title_list.append
    ...
    try:
        ...
        season_regex = ()
        season_regex += tuple(
            [r"complete[.-]%s[.-]season" % x for x in season_ordinal_list]
        )
        season_regex += tuple(
            [r"complete[.-]%s[.-]season" % x for x in season_ordinal2_list]
        )
        ...
        while season_count <= int(total_seasons):
            to_season_ranges.append(
                start_season + ".to.%s" % "0" + str(season_count)
                if int(season_count) < 10
                else start_season + ".to.%s" % str(season_count)
            )
            season_count += 1
        if any(i in dot_release_title for i in to_season_ranges):
            keys = [i for i in to_season_ranges if i in dot_release_title]
        ...
        while season_count <= int(total_seasons):
            to_season_ranges.append(
                start_season + ".to.s%s" % "0" + str(season_count)
                if int(season_count) < 10
                else start_season + ".to.s%s" % str(season_count)
            )
            season_count += 1
        if any(i in dot_release_title for i in to_season_ranges):
            keys = [i for i in to_season_ranges if i in dot_release_title]
...
def info_from_name(
    release_title, title, year, hdlr=None, episode_title=None, season=None, pack=None
):
    try:
...
def clean_name(release_title):
    try:
        ...
        if releasetitle_startswith("rifftrax"):
        ...
        for i in unwanted_tags:
            if releasetitle_startswith(i):
                ...
                release_title = re.sub(
                    r"^%s" % re.escape(i), "", release_title, 1, re.I
                )
...
def _size(siz):
    try:
        ...
        if dec_count == 2:
            siz = siz.replace(
                ".", ",", 1
            )  # torrentproject2 likes to randomly use 2 decimals vs. a comma then a decimal
        float_size = round(
            float(re.sub(r"[^0-9|/.|/,]", "", siz.replace(",", ""))) / div, 2
        )  # comma issue where 2,750 MB or 2,75 GB (sometimes replace with "." and sometimes not)
...
def base32_to_hex(hash, caller):
    ...
    log_utils.log(
        '%s: base32 hash  "%s"  converted to hex 40  "%s" ' % (caller, hash, hex),
        __name__,
        log_utils.LOGDEBUG,
    )
    return hex
...
def is_host_valid(url, domains):
    try:
        if any(
            x in url.lower() for x in (".rar.", ".zip.", ".part.", ".sample.")
        ) or any(
