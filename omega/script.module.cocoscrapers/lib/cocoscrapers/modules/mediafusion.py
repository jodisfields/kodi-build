...
class MediaFusion:
    ...
    def auth(self):
        try:
            ...
            response = requests.post(
                urljoin(self.base_link, "kodi/generate_setup_code"),
                json=data,
            ...
            data.get("configure_url")
            progressDialog = control.progressDialog
            progressDialog.create(
                "Mediafusion",
                control.progress_line
                % (
                    "Open this link in a browser : %s" % data.get("configure_url"),
                    "Kodi Code: %s" % code,
                    "",
                ),
            )
            try:
                ...
                while not progressDialog.iscanceled() and time_passed < expires_in:
                    try:
                        ...
                        log_utils.log(
                            "Request Error: %s" % str(e), __name__, log_utils.LOGDEBUG
                        )
                        ...
                        if not response:
                            ...
                            if response.status_code == 200:
                                data = response.json()
                                control.setSetting(
                                    "mediafusion.userdata", data.get("secret_string")
                                )
                                control.openSettings(
                                    "1.24", "script.module.cocoscrapers"
                                )
                                return control.notification(
                                    message="Successfully added custom MediaFusion url."
                                )
            finally:
    ...
    def clear(self):
        ...
        control.openSettings("1.24", "script.module.cocoscrapers")
        return control.notification(
            message="Successfully cleared MediaFusion user data."
        )
