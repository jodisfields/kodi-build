...
# Reusable opener for connection pooling
_default_opener = None


def _get_default_opener():
...
def request(
    ...
    try:
        ...
        try:
            ...
            try:
                ignore = ignoreErrors and (
                    int(response.code) == ignoreErrors
                    or int(response.code) in ignoreErrors
                )
            except Exception:
                ignore = False
            ...
            if not ignore:
                if response.code in (
                    ...
                    try:
                        ...
                        encoding = None
                    if encoding == "gzip":
                        cf_result = gzip.GzipFile(fileobj=BytesIO(cf_result)).read()
                    ...
                    if flare and "cloudflare" in str(response.info()).lower():
                        ...
                        try:
                            ...
                            ):  # if cfscrape server still responds with 403
                                log_utils.log(
                    ...
                    if error is False:
                        ...
                        error is True and response.code in (401, 404, 405)
        ...
        if (
            not as_bytes and "Blazingfast.io" in result and "xhr.open" in result
        ):  # who da fuck?
...
def _basic_request(
    url, headers=None, post=None, method="GET", timeout="30", limit=None, ret_code=None
):
    try:
        try:
            headers.update(headers)
...
def _replaceHTMLCodes(txt):
    try:
        ...
        txt = re.sub(
            r"(&#[0-9]+)([^;^0-9]+)", "\\1;\\2", txt
        )  # fix html codes with missing semicolon
        txt = unescape(txt)
        txt = txt.replace("&quot;", '"')
        txt = txt.replace("&amp;", "&")
        txt = txt.replace("&lt;", "<")
...
def randomagent():
    ...
    ]  # (compatible, MSIE) removed, dead browser may no longer be compatible and it fails for glodls with "HTTP Error 403: Forbidden"
...
class cfcookie:
    ...
    def get_cookie(self, netloc, ua, timeout):
        try:
            ...
            init = re.findall(
                r"setTimeout\(function\(\){\s*.*?.*:(.*?)};", result, re.I
            )[-1]
