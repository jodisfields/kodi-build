...
def setting(id, fallback=None):
    try:
        settings_dict = jsloads(homeWindow.getProperty("cocoscrapers_settings"))
    except Exception:
...
def make_settings_dict():  # service runs upon a setting change
    try:
        ...
        curSettings = root.getElementsByTagName(
            "setting"
        )  # minidom instead of element tree
        settings_dict = {}
...
def refresh_debugReversed():  # called from service "onSettingsChanged" to clear cocoscrapers.log if setting to reverse has been changed
    if homeWindow.getProperty("cocoscrapers.debug.reversed") != setting(
        "debug.reversed"
    ):
        homeWindow.setProperty("cocoscrapers.debug.reversed", setting("debug.reversed"))
        execute(
            "RunPlugin(plugin://script.module.cocoscrapers/?action=tools_clearLogFile)"
        )
...
def isVersionUpdate():
    ...
    try:
        ...
        xbmc.log(
            "CocoScrapers Addon Data Path Does not Exist. Creating Folder....", LOGINFO
        )
        addon_folder = transPath(
            "special://profile/addon_data/script.module.cocoscrapers"
        )
...
def clean_settings():
    ...
    try:
        ...
        for item in root:
            ...
            # setting_value = item.text
            setting_id = item.getAttribute("id")  # minidom instead of element tree
            setting_default = item.getAttribute(
                "default"
            )  # minidom instead of element tree
            try:
                setting_value = item.firstChild.data  # minidom instead of element tree
        ...
        sleep(200)
        notification(
            title=addon_name, message=lang(32042).format(str(len(removed_settings)))
        )
    except Exception:
        from cocoscrapers.modules import log_utils
...
def getProviderDefaults():
    provider_defaults = {}
    try:
        for item in mdParse(SETTINGS_PATH).getElementsByTagName(
            "setting"
        ):  # holy shit look at that.
            setting_id = item.getAttribute("id")  # minidom instead of element tree
            if not setting_id.startswith("provider."):
                continue
            try:
                defaulttext = item.getElementsByTagName("default")[0].firstChild.data
            except Exception:
                defaulttext = "false"
