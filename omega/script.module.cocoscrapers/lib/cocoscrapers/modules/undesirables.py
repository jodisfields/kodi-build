...
class Undesirables:
    def get_enabled(self):
        ...
        results = self.dbcur.execute(
            "SELECT keyword FROM undesirables WHERE enabled = ?", (True,)
        )
    ...
    def get_default(self):
        self.make_database_objects()
        results = self.dbcur.execute(
            "SELECT keyword FROM undesirables WHERE user_defined = ?", (False,)
        )
        return self.process_keywords(results)
    ...
    def get_user_defined(self):
        self.make_database_objects()
        results = self.dbcur.execute(
            "SELECT keyword FROM undesirables WHERE user_defined = ?", (True,)
        )
        return self.process_keywords(results)
    ...
    def set_many(self, undesirables):
        ...
        self.dbcur.executemany(
            "INSERT OR REPLACE INTO undesirables VALUES (?, ?, ?)", undesirables
        )
        self.dbcon.commit()
        self.dbcon.close()
    ...
    def remove_many(self, undesirables):
        self.make_database_objects()
        self.dbcur.executemany(
            "DELETE FROM undesirables WHERE keyword = ?", undesirables
        )
        self.dbcon.commit()
        self.dbcon.close()
...
def undesirablesSelect():
    ...
    [i for i in UNDESIRABLES if i not in enabled]
...
def undesirablesUserRemove():
    ...
    if not user_undesirables:
        return notification(message=32084)
...
def undesirablesInput():
    ...
    if user_defined:
        ...
        current_user_string = ""
    undesirables_string = dialog.input(
        heading=lang(32048), defaultt=current_user_string
    )
    if not undesirables_string:
        return
