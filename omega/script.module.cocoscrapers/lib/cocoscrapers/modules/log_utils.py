...
import inspect
from cocoscrapers.modules.control import (
    transPath,
...
def log(msg, caller=None, level=LOGINFO):
    ...
    if isinstance(msg, int):
        msg = lang(msg)  # for strings.po translations
    try:
        ...
        xbmc.log(
            "[ script.module.cocoscrapers ] log_utils.log() Logging Failure: %s" % (e),
            LOGERROR,
        )
...
def error(message=None, exception=True):
    try:
        ...
        xbmc.log(
            "[ script.module.cocoscrapers ] log_utils.error() Logging Failure: %s"
            % (e),
            LOGERROR,
        )
...
def clear_logFile():
    ...
    try:
        ...
        xbmc.log(
            "[ script.module.cocoscrapers ] log_utils.clear_logFile() Failure: %s"
            % (e),
            LOGERROR,
        )
        cleared = False
...
def view_LogFile(name):
    try:
        ...
        windows = TextViewerXML(
            "textviewer.xml", addonPath(), heading=heading, text=text
        )
        windows.run()
        del windows
    except Exception:
...
def view_TorrentStats(name):
    try:
        ...
        f.close()
        stats_lines = "\n".join(
            [line for line in text.splitlines() if "#STATS" in line]
        )
        heading = "[B]%s -  LogFile[/B]" % name
        windows = TextViewerXML(
            "textviewer.xml", addonPath(), heading=heading, text=stats_lines
        )
        windows.run()
        del windows
    except Exception:
    ...
    try:
        ...
        if "key" in response.json():
            ...
            supported_platform = any(
                value in sys_platform for value in ("win32", "linux2")
            )
            ...
            if supported_platform:
                list += [
                    (
                        "[COLOR %s]  -- Copy url To Clipboard[/COLOR]"
                        % highlight_color,
                        " ",
                    )
                ]
            ...
            notification(
                message="CocoScrapers Log upload failed: %s"
                % str(response.json()["message"])
            )
            log(
                "CocoScrapers Log upload failed: %s" % str(response.json()["message"]),
                level=LOGERROR,
            )
        else:
...
def normalize(msg):
    try:
        ...
        msg = "".join(
            c
            for c in unicodedata.normalize("NFKD", msg)
