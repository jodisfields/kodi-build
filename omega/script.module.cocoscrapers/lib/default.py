...
if action == "CocoScrapersSettings":
    ...
    control.execute(
        "RunPlugin(plugin://script.module.cocoscrapers/?action=toggleAllTorrent&amp;setting=false)"
    )
    control.sleep(500)
    sourceList = []
    ...
    if cleared == "canceled":
        ...
        control.notification(message="CocoScrapers Log File Successfully Cleared")
    else:
        control.notification(
            message="Error clearing CocoScrapers Log File, see kodi.log for more info"
        )
